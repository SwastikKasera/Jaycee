      ...
      <div className=" w-fit flex justify-center items-center gap-2 text-md">
        {NavItem.map((item, index) =>
          item?.menu ? (
            ...
            <Link to={`${item.url}`} className="text-white px-3 py-1 rounded-full cursor-pointer transition-colors duration-150 hover:bg-accent hover:text-secondary" key={index}>{item.text}</Link>
          )
        )}
      </div>
